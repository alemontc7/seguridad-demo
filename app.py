...
def realizarPedido(total):
    ...
    if request.method == 'POST':
        ...
        db.Pedidos.insert_one({"idCliente":ObjectId(idCliente), "email": correoCliente, "fechaHora":fechaHora, "nota":nota, "tipoDePago":tipoDePago, "detallePedido":detallePedido, "montoTotal":montoTotal })
        db.Carrito.delete_one({"idCliente":ObjectId(idCliente)})
...
def verMiCuenta():
    ...
    return render_template('verCuenta.html',usuario=usuarioSolicitado, clientes = clientess,correoClienteRecibido=correoCliente, user=True)

##########DEL ALE 6########################

@app.route('/actionUpdateCuenta', methods=['POST'])
def actionUpdateCuenta():
    resultat = request.form
    nombre = request.values.get("nombre")
    apellido = request.values.get("apellido")
    email = request.values.get("email")
    ...
    #usuarios.insert_one({ "nombre":nombre,  "apellido":apellido, "direccion":direccion,"email":email, "contrasenia":contrasenia, "estadoDeCuenta":True, "celular":celular, "favoritos": []})
    filter = {'email': email}
    newvalues = { "$set": { 'nombre': nombre, 'apellido':apellido, 'contrasenia': contrasenia, 'celular': celular, 'direccion': direccion } }
    res = usuarios.update_one(filter, newvalues)
    ...
    return render_template('verCuenta.html', user=True, usuario=usuarioSolicitado, clientes = clientess,correoClienteRecibido=correoCliente, exito=True )



################DEL ALE 7##########################
@app.route('/verFavoritos')
def verFavoritos():
    email =  session.get("usuario")
    pipeline = [{'$unwind': "$favoritos"},{
        "$match": {"email": email}},
        {'$project': {"favoritos": 1, "_id": 0}},
        {'$lookup':{ 'from': 'Productos','foreignField': '_id','localField': 'favoritos','as': 'favoritos'}},
        {'$unwind': '$favoritos'},
        {'$project': 
        {"_id": "$favoritos._id", "nombre": "$favoritos.nombre", 
        "categoria": "$favoritos.categoria", "precio": "$favoritos.precio", "descripcion": "$favoritos.descripcion"}}]
    print(pipeline)
    listaDeFavoritos =  db.Usuarios.aggregate(pipeline)
    listaaux = []
    ...
    return render_template('verFavoritos.html', prods = listaaux,correoClienteRecibido=correoCliente)
...
def verPedidos():
    ...
    consultaCliente = list(db.Usuarios.find({'email':correoCliente},{'_id':1}))
    idCliente = consultaCliente[0].get('_id')
    listaPedidos = pedidos.find({"idCliente": ObjectId(idCliente)})
    return render_template('verPedidos.html', pedidos = listaPedidos,correoClienteRecibido=correoCliente)


######################## DEL ALE 9 ################################
@app.route('/verPedido/<_idSolicitado>')
def verPedido(_idSolicitado):
    correoCliente = str(session['usuario'])
    listaProductos = db.Pedidos.aggregate([{'$match': {"_id": ObjectId(_idSolicitado)}}, {'$project': {'_id': 0, "detallePedido":1}}, 
    {'$unwind': "$detallePedido"},
    {'$project': {"idProducto": "$detallePedido.idProducto", "cantidad": "$detallePedido.cantidad", "subtotal": "$detallePedido.subtotal"}},
    {'$lookup':
        { 'from': 'Productos',
            'foreignField': '_id',
    ...
    {'$unwind': "$prod"},
    {'$project': {"cantidad": 1, "subtotal":1 , "nombre": "$prod.nombre"}}
    ])
    listaaux = []
    for i in listaProductos:
        print(i)
        listaaux.append(i)
    return render_template('verPedido.html', productos=listaaux, correoClienteRecibido=correoCliente)
